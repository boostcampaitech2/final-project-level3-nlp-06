...
def make_datasets(dartdataset, newsdataset):
    ...
    tokenizer = Mecab()
    tot = []
    for txt in list(pre_dart.values()):
...
def get_corp(split_news, dart_dict, bm25, pre_dart, pre_name):
    random.seed(2021)
    news_ids = random.sample(range(len(split_news)), 20)
    return_list = []
    
    for target_idx in news_ids:
        ...
        if max(doc_scores) > 2:
            ...
            for dart in list_dart_n:
                corporation_names.append(pre_name[dart])
            if len(corporation_names) == 3:
                return_list.append({'news' : {'idx': target_idx, 'data': " ".join(target)}, 'keywords': ', '.join(tokenized_query), 'corporations': ','.join(corporation_names)})
        """
...
if __name__ == '__main__':
    ...
    get_corp(split_news, dart_dict, bm25, pre_dart, pre_name)
